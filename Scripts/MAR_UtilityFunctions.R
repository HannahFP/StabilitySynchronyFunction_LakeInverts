# Define the estMARcomm_lv_overdisp function
estMARcomm_lv_overdisp <- function(Y, nr_replicates, X = NULL, n_d = 2, n_lv=3, nu=1,
  ...
  # Determine the family code based on the family parameter
  if (is.null(family)) stop("'family' not specified")
  
  if (a_i_Random && a_i_All_Equal) stop("a_i : a_i_Random==TRUE and a_i_All_Equal==TRUE ???")
  if (c_i_Random && c_i_All_Equal) stop("c_i : c_i_Random==TRUE and c_i_All_Equal==TRUE ???")
  ...
  n_s <- nrow(Y)  # Number of species
  n_t <- length(nr_replicates)  # Number of time points
  
  prms <- list()  # Parameters list
  map <- list()  # Mapping for parameters
  
  # Initial estimates
  a_i_hat <- rep(0.5, n_s)
  c_i_hat <- as.numeric(rowMeans(log(Y + 1), na.rm = TRUE) * (1 - a_i_hat) - log(nu))
  
  # Handle random effects for a_i
  if (a_i_Random) {
  ...
  # Prepare the initial values for the X parameter
  ...
  # Step 2: Refine the model with additional parameters
  cat("Step 2 start: ", "\t")
  ...
  # Final results extraction and reporting
  out_structure <- structure(list(data = data_list))
...
# Define the extractEstimates function
